...
export class ProgressRing extends HTMLElement {
	...
	redraw(percent) {
		...
		let progress_ring = SVG.put(svg_root, SVG.make("circle", "progress-ring", [], this.ring_attributes(normalizedRadius, radius,this.circumference, stroke) ), 1, true);
		let status_text   = SVG.put(svg_root, SVG.make("text",   "status_text",   [], this.status_attributes(radius),'', status ), 2, true);
		SVG.style(progress_ring , this.ring_style(radius, this.circumference )); 
