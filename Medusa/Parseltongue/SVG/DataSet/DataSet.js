...
export class DataSet extends Meta{
    ...
    static calculate_positions(points, width, height, xbox, ybox){
        return points.map(e => {
            const x = DataSet.map(e[0], xbox.minimum, xbox.maximum , 0, width.range())
            const y = DataSet.map(e[1], ybox.minimum, ybox.maximum , height.range(), 0)
            return [x, y]
        });
    }
    ...
    static calculate_positions(points, xbox, ybox, xspread, yspread){
        return points.map(e => {
            xspread.current = e[0];
            yspread.current = e[1]
            const x = DataSet.project(xbox, 'max', xspread, 'max');
            const y = DataSet.project(ybox, 'mid', yspread, 'mid');
            return [x, -y]
        });
    }
    ...
    static control_point(current, previous, next, reverse, flatness, smoothness) {
        ...
        const la = DataSet.length_angle(p, n);
        // work in progress…
        // const flat = DataSet.map(Math.cos(la.angle) * this.flattening_w.current, 0, 1, 1, 0)
        const flat = DataSet.map(Math.cos(la.angle) * flatness, 0, 1, 1, 0)
    ...
    static bezier_command(point, i, a, flatness, smoothness) {
        const cps = DataSet.control_point(a[i - 1], a[i - 2], point, false, flatness, smoothness);
        const cpe = DataSet.control_point(point, a[i - 1], a[i + 1], true, flatness, smoothness);
                ...
                function(acc, e, i, a) {
                    return i === 0 ? 
                        ...
                        `M ${0},${0} L ${e[0]},${0} L ${e[0]},${e[1]}` : 
                            `${acc} ${ DataSet.bezier_command(e, i, a, flatness, smoothness)}`} 
                                    , "");
    }
    ...
    add_scope(name, scope, units, start_index, path_smoothing, path_flattening, path_color, flag_color, values){
        ...
        // if(this.scope_root==null) { this.scope_root = SVG.make("g", "scope_root", [], {}); }
        // new Scope(null, name, values, this /* this.scopes */, this.svgroot,  {svg: this.scope_root}, null, null, false, scope, units, start_index, path_smoothing, path_flattening, path_color, flag_color);
    ...
    reload(){
        ...
        for (let index in this.scopes) {
            ...
            [this.xspread, this.yspread] = DataSet.calculate_spread(scope.W, this.xspread, this.yspread);
        }
        ...
        // this.positions = DataSet.calculate_positions(this.W, this.xbox_w, this.ybox_w, this.xspread, this.yspread);
        // let path_data =  DataSet.path_d(this.positions, this.flattening_w.current, this.smoothing_w.current);
        // if(this.path) {
        ...
        //     flag.addEventListener("mouseover", DataSet.showPopup);
        //     flag.addEventListener("mouseout", DataSet.hidePopup);
	...
	constructor(parent, name, collectors, svgroot, realm, svg, def_ref, draggable, smoothing, flattening){
        const scopes = [
            ...
            {   name: "two",
                ...
                units: 7,
                start_index: 0,
                attributes: { line:   { smoothing: 0.15, flattening: 0.5 }, colors: { path: "rgba(55, 165, 230, 1.0)", flags: "blue" },},
        ...
        new Boundary(this, 'xbox',       null, null,svgroot, null, svg, def_ref, false,  -200, 0, 0, 8, 0);
        new Boundary(this, 'ybox',       null, null,svgroot, null, svg, def_ref, false, -100, 0, 100, 8, 0);
