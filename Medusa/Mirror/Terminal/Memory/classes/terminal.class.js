class Terminal {
    constructor(opts) {
        if (opts.role === "client") {
            ...
            this.xTerm = import("xterm").Terminal;
            const {AttachAddon} = import("xterm-addon-attach");
            const {FitAddon} = import("xterm-addon-fit");
            const {LigaturesAddon} = import("xterm-addon-ligatures");
            const {WebglAddon} = import("xterm-addon-webgl");
            this.Ipc = import("electron").ipcRenderer;
            ...
            this.Pty = import("node-pty");
            this.Websocket = import("ws").Server;
            this.Ipc = import("electron").ipcMain;
            ...
            this._getTtyCWD = tty => {
                return new Promise((resolve, reject) => {
                    let pid = tty._pid;
                    switch(import("os").type()) {
                        case "Linux":
                            import("fs").readlink(`/proc/${pid}/cwd`, (e, cwd) => {
                                if (e !== null) {
                                    reject(e);
                                } else {
                                    resolve(cwd);
                            ...
                            break;
                        case "Darwin":
                            import("child_process").exec(`lsof -a -d cwd -p ${pid} | tail -1 | awk '{ for (i=9; i<=NF; i++) printf "%s ", $i }'`, (e, cwd) => {
                                if (e !== null) {
                                    reject(e);
                                } else {
                                    resolve(cwd.trim());
            ...
            this._getTtyProcess = tty => {
                return new Promise((resolve, reject) => {
                    ...
                    switch(import("os").type()) {
                        case "Linux":
                        case "Darwin":
                            import("child_process").exec(`ps -o comm --no-headers --sort=+pid -g ${pid} | tail -1`, (e, proc) => {
                                if (e !== null) {
                                    reject(e);
                                } else {
                                    resolve(proc.trim());
