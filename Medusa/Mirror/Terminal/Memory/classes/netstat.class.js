class Netstat {
    constructor(parentId) {
        ...
        this._httpsAgent = new import("https").Agent({
            keepAlive: false,
            maxSockets: 10
        });
        ...
        let geolite2 = import("geolite2-redist");
        let maxmind = import("maxmind");
        geolite2.downloadDbs(import("path").join(import("@electron/remote").app.getPath("userData"), "geoIPcache")).then(() => {
        ...
        window.si.networkInterfaces().then(async data => {
            ...
            if (net.ip4 === "127.0.0.1") {
                ...
                if (this.runsBeforeGeoIPUpdate === 0 && this.lastconn.finished) {
                    this.lastconn = import("https").get({host: "myexternalip.com", port: 443, path: "/json", localAddress: net.ip4, agent: this._httpsAgent}, res => {
                        let rawData = "";
                        res.on("data", chunk => {
                            rawData += chunk;
                        });
