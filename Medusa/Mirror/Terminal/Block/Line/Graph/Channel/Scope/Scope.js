...
export class Scope extends Meta{
	...
	static project(b1, a1, b2, a2, flip=false) { 
    ...
    static calculate_positions(points, xbox, ybox, xspread, yspread){
        return points.map(e => {
            ...
            const x = Scope.project(xbox, 'min', xspread, 'min');
            const y = Scope.project(ybox, 'min', yspread, 'min',true);
    ...
    static bezier_command(point, i, a, flatness, smoothness, open=true) {
        ...
        const close = i === a.length - 1 ? (" z"?!open:"") : "";
        return `C ${cps[0]},${cps[1]} ${cpe[0]},${cpe[1]} ${point[0]},${point[1]}${close}`;
    }

    static path_d(positions, flatness, smoothness) {//https://yqnn.github.io/svg-path-editor/
        return positions.
            reduce(
                function(acc, e, i, a) {
                    return i === 0 ? 
    ...
    styles() {
        return {
            path: { fill:'none',/* fill: this.path_color, */ stroke: this.path_color, strokeWidth: 0.5, /* fillOpacity: 0.15, */ strokeOpacity: 0.8 },
    ...
    reload(){
        ...
        if(this.W.length){
            let set_container = SVG.put(this.set_root, SVG.make("g", "set_container", [], {}), 0/* set_index */, true);
            let path_data =  Scope.path_d(this.positions, this.path_flattening, this.path_smoothing)
            
            let path = SVG.put(set_container, SVG.make('path', 'path_set', [], {}), 0, true);
            SVG.configure(path , Object.assign({d:path_data}, this.styles().path ));
            this.path = path;
            ...
            for (let index in this.positions) {
                ...
                SVG.configure(flag , Object.assign({cx:position[0], cy:position[1], r:2.5, title:this.name}, this.styles().flags ));
