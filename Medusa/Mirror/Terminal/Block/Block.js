...
class Scale {
    constructor(owner, element) { //https://codepen.io/zhangbao/pen/gXWZRJ // https://codepen.io/ronm/pen/qmpaKJ
        this.owner = owner
        var pos1 = 0, pos2 = 0, pos3 = 0, pos4 = 0;
            ...
            if (!isDown) { // 鼠标未按下
                ...
                document.onmousemove = function (ev) {
                    ...
                    if (xResize=== '') { // 向上拖动
                        ...
                        ev = ev || window.event;
                        ev.preventDefault();
                        pos1 = pos3 - ev.clientX; // calculate the new cursor position:
...
export class Block {
    ...
    transpose(row){
        // this.node.transpose(row)
    }

    display_terminal(display){
        HTML.style(this.terminal.container, {  'display': !display ? 'none' : 'block'})
    }
    
    box(wrapper) {
        let [root, box] = HTML.chain(wrapper, `div::`); 
        HTML.style(box, { background: 'black', height: '500px', position: 'fixed', width: '500px' })
        HTML.style(box, { top:  'calc(50% - 250px)', left: 'calc(50% - 250px)'})
        // HTML.style(box, { top: '50%', left: '50%', transform: 'translateX(-50%) translateY(-50%)', })
        HTML.style(box, { 'border-radius': '10px', 'border-color': 'white', 'border-width': '2px', 'border-style':'solid' })
        ...
        HTML.style(box, {  'overflow': 'hidden'})
        
        // new Moveable(box);
        this.dragger = new Drag(this, box)
        this.scaler = new Scale(this, box)

		let [settings] = HTML.ladder(box,`div:settings:`)
		let [thumb, move, terminal, transposer] = HTML.ladder(settings, `img:eye:/img:eye:/img:eye:/img:eye:`);
		...
		HTML.configure(terminal, {src:"assets/icons/svg+xml/terminal.svg"}) // https://stackoverflow.com/questions/7415872/change-color-of-png-image-via-css
		HTML.configure(transposer, {src:"assets/icons/svg+xml/more-vertical.svg"}) // https://stackoverflow.com/questions/7415872/change-color-of-png-image-via-css
		...
		terminal.addEventListener("click", ((e)=>{
            this.flags.no_terminal = !this.flags.no_terminal
			e.target.src = this.no_terminal? "assets/icons/svg+xml/terminal.svg" : "assets/icons/svg+xml/terminal.svg"
            // HTML.style(this.terminal.container, {  'display': this.flags.no_terminal ? 'none' : 'block'})
            this.display_terminal(!this.flags.no_terminal)
		}).bind(this))
		transposer.addEventListener("click", ((e)=>{
            this.flags.transposed_row = !this.flags.transposed_row
			e.target.src = this.flags.transposed_row? "assets/icons/svg+xml/more-vertical.svg" : "assets/icons/svg+xml/more-horizontal.svg"
            // this.fire('resized', box)
            this.transpose(!this.flags.transposed_row)
		}).bind(this))
    ...
    reload(){
        var vterminal = HTML.make("viewer-terminal","", [], {})
        this.terminal = vterminal
        this.node.append(vterminal)
    }
    ...
    constructor(terminal, width, height) {
        ...
        this.flags = {}
        this.flags.no_terminal = false
        this.flags.transposed_row = true
