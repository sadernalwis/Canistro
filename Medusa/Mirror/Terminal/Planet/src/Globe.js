...
var 
    // TWEEN = import('tween.js'),
    // THREE = import('three'),
    Hexasphere = import ('./hexasphere/hexasphere.js'),
    // Quadtree2 = import('./quadtree/quadtree2.js'),
    Quadtree2 = import('./quadtree/qt.js'),
    Vec2 = import('./vec2/vec2.js'),
    Pin = import('./Pin'),
    Marker = import('./Marker'),
    Satellite = import('./Satellite'),
    SmokeProvider = import('./SmokeProvider'),
    pusherColor = import('../include/pcolor.js'),
    utils = import('./utils');
window.Colors = pusherColor
...
var createParticles = function(){
    ...
    // geometry.setAttribute( 'index', Uint16Array, triangles * 3, 1 );
    // geometry.setAttribute( 'position', Float32Array, triangles * 3, 3 );
    // geometry.setAttribute( 'normal', Float32Array, triangles * 3, 3 );
    // geometry.setAttribute( 'color', Float32Array, triangles * 3, 3 );
    // geometry.setAttribute( 'lng', Float32Array, triangles * 3, 1 );
    // geometry.setAttribute( 'index', new THREE.BufferAttribute(new Uint16Array(triangles * 3), 1));
    geometry.setAttribute( 'index', new THREE.BufferAttribute(new Uint32Array(triangles * 3), 1));
    geometry.setAttribute( 'position', new THREE.BufferAttribute(new Float32Array(triangles * 3), 3));
    geometry.setAttribute( 'normal', new THREE.BufferAttribute(new Float32Array(triangles * 3), 3));
    geometry.setAttribute( 'color', new THREE.BufferAttribute(new Float32Array(triangles * 3), 3));
    ...
    var lng_values = geometry.getAttribute("lng").array;
    // var baseColorSet = pusherColor(this.baseColor).hueSet();
    var baseColorSet = [Colors("darkgrey"), Colors("darkslategrey"), Colors("dimgrey"), Colors("grey"), Colors("lightgrey"), Colors("lightslategrey"), Colors("slategrey"),
    Colors("antiquewhite"),Colors("floralwhite"),Colors("ghostwhite"),Colors("navajowhite"),Colors("white"),Colors("whitesmoke")]
    // baseColorSet = [0.1]
    // console.log(baseColorSet)
    var myColors = [];
    ...
    var addTriangle = function(k, ax, ay, az, bx, by, bz, cx, cy, cz, lat, lng, color){
        ...
        colors[ i + 7 ] = color.g;
        colors[ i + 8 ] = color.b; };
    var chunk = 15,chunk_size = 100;
    var chunks = parseInt(this.tiles.length/chunk_size)
    var start_steps = 0, step = 10, end_steps = 1;
    var start_i = chunk*chunk_size, end_i = (chunk+1)*chunk_size
    end_i = Math.min(this.tiles.length, end_i)
...
var createIntroLines = function(){
    ...
    for(var i = 0; i<this.introLinesCount; i++){
        ...
        for(var j = 0; j< lenBase; j++){
            ...
            positions.push(thisPoint.x*this.introLinesAltitude);
            positions.push(thisPoint.y*this.introLinesAltitude);
            positions.push(thisPoint.z*this.introLinesAltitude); }
...
Globe.prototype.init = function(cb){ /* public globe functions */
    ...
    this.smokeProvider = new SmokeProvider(this.scene); // create the smoke particles
