...
import Hexasphere from './src/hexasphere/hexasphere.js';

export class HexaSphere{

    geodesic_polyhedron(wrapper){
        ...
        var isLand = function(lat, lon){
            ...
            if(pixelData == null){ pixelData = projectionContext.getImageData(0,0,img.width, img.height); }
        ...
        var createScene = function(radius, divisions, tileSize){
            ...
            for(var i = 0; i< hexasphere.tiles.length; i++){
                ...
                if(geometry.vertices.length > 5){ geometry.faces.push(new THREE.Face3(0,4,5)); }
                if(isLand(latLon.lat, latLon.lon)){ material = meshMaterials[Math.floor(Math.random() * meshMaterials.length)] } 
                else { material = oceanMaterial[Math.floor(Math.random() * oceanMaterial.length)] }
    ...
    constructor(terminal, width, height) {
        ...
        this.hexasphere = this.geodesic_polyhedron(this.block.node)
